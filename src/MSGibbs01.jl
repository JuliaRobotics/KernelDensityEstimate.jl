...
"""
function sampleIndices!(X::Array{Float64,2},
                        cmoi::MSCompOpt,
                        glb::GbGlb,
                        offset::Int, # offset
                        diffop=(-,)  )
...
"""
function samplePoint!(X::Array{Float64,2},
                      glb::GbGlb,
                      idx::Int,
                      addop::Tuple=(+,),
...
function gibbs1(Ndens::Int, trees::Array{BallTreeDensity,1},
  ...
  glbs.p = zeros(maxNp)
  glbs.Malmost = zeros(glbs.Ndim, 1)
  ...
  for s in 1:Np
    ...
    for l in 1:glbs.Nlevels
      ...
      samplePoint!(glbs.newPoints, glbs, s, addop, getMu, getLambda ) # frm

      # step a level down in the tree
      levelDown!(glbs);

      # ??
      sampleIndices!(glbs.newPoints, cmoi, glbs, s, diffop); frm
    ...
    samplePoint!(glbs.newPoints, glbs, s, addop, getMu, getLambda, addEntropy); # frm
  end
  glbs = 0
  nothing
end
